...
const ServiceAssetLinker: React.FC<Props> = ({
            ...
            try {
                ...
                if (reposRes.ok) {
                    ...
                    const repoNames = Array.isArray(reposData)
                        ? reposData.map((r: any) => r.repository || r).filter(Boolean)
                        : ['Web', 'SEO', 'SMM'];
    ...
    // Debug logging
    React.useEffect(() => {
        console.log('[ServiceAssetLinker] Props received:', {
            availableAssets: availableAssets?.length || 0,
            linkedAssets: linkedAssets?.length || 0,
            totalAssets,
            repositoryFilter
        });
    }, [availableAssets, linkedAssets, totalAssets, repositoryFilter]);
                        ...
                        <div className="flex flex-col">
                            ...
                            <div className="flex-1 bg-gradient-to-br from-blue-50 to-cyan-50 rounded-b-2xl border-2 border-blue-200 border-t-0 p-4 min-h-[600px] max-h-[700px]">
                                <div className="h-full overflow-y-auto space-y-3 pr-2 custom-scrollbar">
                                    ...
                                    ) : (availableAssets && availableAssets.length > 0) ? availableAssets.map(asset => (
                                        <div
                                            key={asset.id}
                                            onClick={() => onToggle(asset)}
