...
const EffortDashboard: React.FC<EffortDashboardProps> = ({ onNavigate }) => {
    ...
    useEffect(() => {
        const handleError = (event: ErrorEvent) => {
            console.error('EffortDashboard error:', event.error);
            setHasError(true);
            setLoading(false);
        };
        ...
        return (
            <div className="h-full flex items-center justify-center bg-slate-50">
                <div className="text-center p-8">
                    ...
                    <p className="text-slate-600 mb-4">Unable to load effort dashboard data.</p>
                    <button 
                        onClick={() => {
                            setHasError(false);
                            fetchDashboardData();
    ...
    const fetchDashboardData = async () => {
        try {
            ...
            const url = `${apiUrl}/dashboards/effort?${queryParams}`;
            
            console.log('Fetching effort dashboard from:', url);
            ...
            console.log('Effort dashboard response:', result);
            ...
            setDashboardData({
                summaryCards: {
                    effortCompletion: { value: 0, target: 0, trend: 'neutral', change: '0%', status: 'loading', lastUpdated: new Date().toISOString() },
                    totalTasksCompleted: { value: 0, target: 0, trend: 'neutral', change: '0 tasks', status: 'loading', lastUpdated: new Date().toISOString() },
                    reworkPercentage: { value: 0, target: 0, trend: 'neutral', change: '0%', status: 'loading', lastUpdated: new Date().toISOString() },
                    qcPassPercentage: { value: 0, target: 0, trend: 'neutral', change: '0%', status: 'loading', lastUpdated: new Date().toISOString() },
                    tatCompliance: { value: 0, target: 0, trend: 'neutral', change: '0%', status: 'loading', lastUpdated: new Date().toISOString() },
                    workloadPrediction: { value: 'N/A', utilizationPercentage: 0, nextWeekForecast: 'N/A', lastUpdated: new Date().toISOString() }
                },
                targetVsActual: [],
                teamHeatmap: [],
                qcPerformance: [],
    ...
    // Ensure dashboardData has all required properties
    ...
    const { summaryCards, targetVsActual, teamHeatmap, qcPerformance, slaMisses, workloadTrends } = safeDashboardData;
