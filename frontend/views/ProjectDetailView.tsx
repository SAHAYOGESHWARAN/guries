...
const ProjectDetailView: React.FC<ProjectDetailViewProps> = ({ projectId, onNavigateBack }) => {
    ...
    const timelineData = useMemo(() => {
        ...
        const projCampaignIds = safeCampaigns.filter(c => c.project_id === projectId).map(c => c.id);
        const safeTasks = Array.isArray(tasks) ? tasks : [];
        const projTasks = safeTasks.filter(t => t.campaign_id && projCampaignIds.includes(t.campaign_id));
