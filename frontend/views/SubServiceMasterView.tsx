        ...
        <div className="space-y-6 animate-fade-in w-full h-full overflow-y-auto p-6 ui-surface">
            <div className="flex justify-between items-start">
                ...
                <div className="flex items-center space-x-3">
                    ...
                    <button
                        ...
                        className="bg-indigo-600 text-white px-4 py-2 rounded-lg text-xs font-bold shadow-sm hover:bg-indigo-700 transition-colors flex items-center gap-1.5"
                    >
                        <span className="text-lg leading-none">+</span> Add New Sub-Service
                    </button>
                </div>
            </div>
            ...
            <div className="bg-white p-4 rounded-xl shadow-sm border border-slate-200">
                    ...
                    <div className="flex flex-wrap items-center gap-3">
                        ...
                        <div className="relative">
                            <select
                                ...
                                className="appearance-none bg-white border border-slate-300 text-sm rounded-lg py-2 pl-3 pr-8 focus:ring-indigo-500 focus:border-indigo-500 cursor-pointer min-w-[160px]"
                            >
                        ...
                        <div className="relative">
                            <select
                                ...
                                className="appearance-none bg-white border border-slate-300 text-sm rounded-lg py-2 pl-3 pr-8 focus:ring-indigo-500 focus:border-indigo-500 cursor-pointer min-w-[150px]"
                            >
                ...
                <div className="flex items-center justify-between text-xs text-slate-500 pt-3 mt-3 border-t border-slate-100">
                    <span>Showing <span className="font-semibold text-slate-700">{filteredData.length}</span> of <span className="font-semibold text-slate-700">{subServices.length}</span> entries</span>
                    {(searchQuery || parentFilter !== 'All Parent Services' || statusFilter !== 'All Status' || contentTypeFilter !== 'All Types') && (
                        <button
                            onClick={() => {
                                setSearchQuery('');
            ...
            <div className="bg-white rounded-xl shadow-sm border border-slate-200 flex flex-col overflow-hidden">
                {loading && (
                    <div className="flex items-center justify-center py-12">
                        <div className="flex items-center gap-3 text-slate-500">
                            ...
                            <span className="text-sm font-medium">Loading sub-services...</span>
                        </div>
                    </div>
                )}
                {!loading && (
                        ...
                        columns={[
                            {
                                ...
                                accessor: (item: SubServiceItem) => (
                                    <span className="font-medium text-slate-800 text-sm">{item.sub_service_name}</span>
                                )
                            },
                            {
                                ...
                                accessor: (item: SubServiceItem) => {
                                    ...
                                    return <span className="text-xs text-slate-700">{parent?.service_name || '—'}</span>;
                                }
                            },
                            {
                                header: 'URL Slug',
                                accessor: (item: SubServiceItem) => (
                                    <span className="font-mono text-xs text-indigo-600">{item.slug || '—'}</span>
                                )
                            },
                            {
                                header: 'Keywords',
                                accessor: (item: SubServiceItem) => {
                                    const keywords = item.focus_keywords || [];
                                    if (keywords.length === 0) return <span className="text-slate-400 text-xs">—</span>;
                                    return (
                                        <span className="text-xs text-slate-600">
                                            {keywords.slice(0, 2).join(', ')}{keywords.length > 2 ? ` +${keywords.length - 2}` : ''}
                                        </span>
                                    );
                                }
                            },
                            ...
                            },
                            {
                                ...
                                accessor: (item: SubServiceItem) => {
                                    ...
                                    return <span className="text-indigo-600 font-medium text-sm">{contentCount + libraryCount}</span>;
                                },
                                className: "text-center"
                            },
                            {
                                header: 'Insights Linked',
                                accessor: (item: SubServiceItem) => {
                                    const count = (item.focus_keywords?.length || 0) + (item.secondary_keywords?.length || 0);
                                    return <span className="text-indigo-600 font-medium text-sm">{count}</span>;
                                },
                                className: "text-center"
                            },
                            {
                            ...
                            },
                            {
                                ...
                                accessor: (item: SubServiceItem) => (
                                    <div className="flex space-x-2">
                                        <button onClick={() => handleView(item)} className="text-slate-500 hover:text-blue-600 font-medium text-xs">View</button>
                ...
                {!loading && filteredData.length === 0 && (
                    <div className="flex flex-col items-center justify-center py-12 text-slate-500">
                        ...
                        <button onClick={handleCreateClick} className="bg-indigo-600 text-white px-4 py-2 rounded-lg text-sm font-bold hover:bg-indigo-700">
                            Create Sub-Service
                        </button>
                    </div>
                )}
