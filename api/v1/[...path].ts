...
const DEFAULT_STORAGE: Record<string, any[]> = {
    assets: [],
    services: [],
    subServices: [],
    users: [
...
// Helper to get collection from KV or return default
async function getCollection(collection: string): Promise<any[]> {
    try {
        const data = await kv.get<any[]>(`${STORAGE_PREFIX}${collection}`);
        return data || DEFAULT_STORAGE[collection] || [];
    } catch (e) {
        console.error(`KV get error for ${collection}:`, e);
        return DEFAULT_STORAGE[collection] || [];
    }
}

// Helper to save collection to KV
async function saveCollection(collection: string, data: any[]): Promise<void> {
    try {
        await kv.set(`${STORAGE_PREFIX}${collection}`, data);
    } catch (e) {
        console.error(`KV save error for ${collection}:`, e);
    }
}
...
export default async function handler(req: VercelRequest, res: VercelResponse) {
    ...
    try {
        ...
        if (fullPath === 'system/stats') {
            return res.status(200).json({ uptime: process.uptime(), memory: process.memoryUsage(), version: '2.5.0' });
        }
...
// Analytics handler
async function handleAnalytics(req: VercelRequest, res: VercelResponse, fullPath: string) {
...
// HR handler
function handleHR(res: VercelResponse, fullPath: string) {
    return res.status(200).json([]);
}
...
// Dashboards handler
async function handleDashboards(res: VercelResponse, fullPath: string) {
...
async function handleCommunication(req: VercelRequest, res: VercelResponse, fullPath: string, method: string) {
...
// Generic CRUD handler with KV persistence
...
async function handleAssetLibrary(req: VercelRequest, res: VercelResponse, fullPath: string, method: string) {
    ...
    if (pathParts.length >= 3) {
        ...
        if (action === 'qc-review' && method === 'POST') {
            return handleQCReview(req, res, assetId, assets);
        }
        if (action === 'submit-qc' && method === 'POST') {
            return handleSubmitForQC(req, res, assetId, assets);
        }
        if (action === 'usage') {
    ...
    if (fullPath === 'assetLibrary/ai-scores' && method === 'POST') {
        return res.status(200).json({
            ...
            analysis: { seo_feedback: 'Good SEO optimization', grammar_feedback: 'Excellent grammar and readability' }
        });
    }
...
async function handleQCReview(req: VercelRequest, res: VercelResponse, assetId: number, assets: any[]) {
    ...
    let newStatus = qc_decision === 'approved' ? 'QC Approved' : qc_decision === 'rejected' ? 'QC Rejected' : 'Rework Required';
    ...
    const notificationText = qc_decision === 'approved'
        ? `Asset "${asset.name}" has been approved!`
        : qc_decision === 'rework' ? `Asset "${asset.name}" requires rework.` : `Asset "${asset.name}" has been rejected.`;
...
async function handleSubmitForQC(req: VercelRequest, res: VercelResponse, assetId: number, assets: any[]) {
    ...
    assets[assetIndex] = {
        ...
        seo_score: seo_score || assets[assetIndex].seo_score,
        grammar_score: grammar_score || assets[assetIndex].grammar_score,
        rework_count: rework_count || assets[assetIndex].rework_count || 0,
        updated_at: new Date().toISOString()
    };
    await saveCollection('assets', assets);
    ...
    notifications.unshift({
        ...
        message: `Asset "${assets[assetIndex].name}" has been submitted for QC review.`,
        text: `Asset "${assets[assetIndex].name}" has been submitted for QC review.`,
        type: 'info',
        read: false,
        is_read: 0,
        created_at: new Date().toISOString(),
...
async function handleAssetUsage(req: VercelRequest, res: VercelResponse, assetId: number, subPath: string[], method: string) {
    ...
    switch (method) {
        case 'GET':
            return res.status(200).json(usageData.filter((u: any) => u.asset_id === assetId));
        case 'POST':
...
async function handleNotifications(req: VercelRequest, res: VercelResponse, fullPath: string, method: string) {
    ...
    switch (method) {
        case 'GET':
            ...
            return res.status(200).json(notifications.map((n: any) => ({
                ...n,
