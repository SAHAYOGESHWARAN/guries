/**
 * Real-time Test Script for Asset Creation Fix
...
async function getTestToken() {
    ...
    if (loginResult.status === 200 && loginResult.data?.token) {
        ...
        console.log('⚠️  Could not authenticate, trying without token\n');
        return null;
    }
}

// Test 1: Create asset with valid application_type
async function testCreateAssetWithValidType() {
    console.log('\n📝 Test 1: Create asset with valid application_type (WEB)');

    const payload = {
        name: 'Test Web Asset - ' + Date.now(),
        type: 'article',
        application_type: 'WEB', // Testing uppercase
        asset_category: 'Blog',
        status: 'Draft',
        created_by: 1,
        keywords: ['test', 'web']
...
async function testCreateAssetWithLowercaseType() {
    console.log('\n📝 Test 2: Create asset with lowercase application_type (seo)');

    const payload = {
        name: 'Test SEO Asset - ' + Date.now(),
        type: 'article',
        application_type: 'seo', // Testing lowercase
        asset_category: 'SEO',
        status: 'Draft',
        created_by: 1
    };
...
async function testCreateAssetWithoutType() {
    console.log('\n📝 Test 3: Create asset WITHOUT application_type (should fail)');
...
async function testCreateAssetWithInvalidType() {
    console.log('\n📝 Test 4: Create asset with INVALID application_type (should fail)');
...
async function testAssetAppearsInList(assetId: number) {
...
async function testSubmitAssetForQC(assetId: number) {
...
async function testAssetAppearsInQCList(assetId: number) {
...
async function testSubmitAssetWithoutScores(assetId: number) {
    console.log('\n📝 Test 8: Submit asset WITHOUT scores (should fail)');

    const payload = {
        submitted_by: 1
        // Missing seo_score and grammar_score
    };
...
async function runTests() {
    ...
    try {
        // Test validation
        await testCreateAssetWithoutType();
        await testCreateAssetWithInvalidType();

        // Test creation with valid types
        const assetId1 = await testCreateAssetWithValidType();
        const assetId2 = await testCreateAssetWithLowercaseType();

        // Test asset appears in list
        if (assetId1) {
            await testAssetAppearsInList(assetId1);
        }

        // Test QC submission
        if (assetId1) {
            await testSubmitAssetForQC(assetId1);
            await testAssetAppearsInQCList(assetId1);
        }

        // Test validation on submission
        if (assetId2) {
            await testSubmitAssetWithoutScores(assetId2);
        }
