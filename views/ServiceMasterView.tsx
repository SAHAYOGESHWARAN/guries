...
import type { Service, SubServiceItem, ContentRepositoryItem, AssetLibraryItem, IndustrySectorItem } from '../types';

const STATUSES = ['All Status', 'Draft', 'In Progress', 'QC', 'Approved', 'Published', 'Archived'];
const INDUSTRIES = ['All Industries', 'Technology', 'Media', 'E-commerce', 'Healthcare', 'Finance', 'Education', 'Manufacturing'];
const SECTORS = ['All Sectors', 'Digital Marketing', 'Content Strategy', 'Social Media', 'Email Automation', 'SEO', 'Web Development', 'Analytics'];
const COUNTRIES = [
...
// Mock data for insights and assets
...
const ServiceMasterView: React.FC = () => {
    const { data: services = [], create, update, remove, refresh } = useData<Service>('services');
    ...
    const { data: industryData = [] } = useData<IndustrySectorItem>('industrySectors');
    ...
    const [selectedCountries, setSelectedCountries] = useState<string[]>(['US', 'UK', 'CA', 'AU', 'DE', 'FR', 'IN', 'SG']);
    ...
    const calculateHealthScore = (service: Service): number => {
        ...
        const optionalFields = [
            service.focus_keywords?.length,
            service.primary_cta_label,
            service.primary_cta_url,
            service.og_title,
            service.og_description
        ].filter(field => field).length;

        return Math.round(((completedFields / requiredFields.length) * 70) + ((optionalFields / 5) * 30));
    };

    // Enhanced services with calculated fields
    const enhancedServices = useMemo(() => {
        return services.map(service => {
            const subServiceCount = subServices.filter(sub => sub.parent_service_id === service.id).length;
            const linkedContentAssets = contentAssets.filter(asset =>
                asset.linked_service_ids?.includes(service.id)
            ).length;
            ...
            // Calculate insights count (placeholder - could be from actual insights data)
            const linkedInsights = Math.floor(Math.random() * 10); // Replace with actual insights data
            ...
            return {
                ...
                asset_count: totalLinkedAssets,
                linked_insights: linkedInsights,
                health_score: healthScore
            };
        });
    }, [services, subServices, contentAssets, libraryAssets]);

    const filteredData = enhancedServices.filter(item => {
        ...
        // Get industry from industry_ids array (assuming first industry)
        const primaryIndustryId = item.industry_ids?.[0];
        const industryRecord = industryData.find(ind => ind.id === parseInt(primaryIndustryId || '0'));
        const itemIndustry = industryRecord?.industry || item.industry || '';
        const itemSector = industryRecord?.sector || '';

        const matchesIndustry = industryFilter === 'All Industries' || itemIndustry === industryFilter;
        const matchesSector = sectorFilter === 'All Sectors' || itemSector === sectorFilter;
        const matchesStatus = statusFilter === 'All Status' || item.status === statusFilter;

        return matchesSearch && matchesIndustry && matchesSector && matchesStatus;
    });
