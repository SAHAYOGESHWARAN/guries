...
const NavLink: React.FC<{ item: NavItem, isChild?: boolean, currentView: string, setCurrentView: (view: string) => void }> = ({ item, isChild = false, currentView, setCurrentView }) => {
  ...
  const baseClasses = "group flex items-center px-3 py-2 text-sm font-medium rounded-lg transition-all duration-200 cursor-pointer mb-1 mx-2";
  ...
  const childClasses = isChild ? "pl-9 text-xs" : "";
...
const Sidebar: React.FC<SidebarProps> = ({ currentView, setCurrentView }) => {
  ...
  return (
    <div className="flex flex-col h-full bg-[#0F172A] border-r border-slate-800 w-[260px] flex-shrink-0 transition-all duration-300 ease-in-out">
      {/* Brand Header */}
      <div className="h-16 flex items-center px-6 border-b border-slate-800 bg-[#0F172A] sticky top-0 z-20">
        <div className="flex items-center gap-3">
            <div className="w-8 h-8 rounded-lg bg-gradient-to-br from-brand-500 to-brand-700 flex items-center justify-center text-white shadow-glow">
                <span className="font-bold text-lg leading-none font-sans">G</span>
            </div>
            <div>
                <h1 className="font-bold text-white text-sm tracking-wide">Guires</h1>
                <p className="text-[10px] text-slate-500 uppercase font-semibold tracking-wider">Marketing OS</p>
            </div>
        </div>
      </div>
